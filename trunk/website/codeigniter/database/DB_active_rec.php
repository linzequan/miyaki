...
class CI_DB_active_record extends CI_DB_driver {
    ...
    public function select_count($select = '', $alias = ''){
        return $this->_max_min_avg_sum($select, $alias, 'COUNT');
    }
    // --------------------------------------------------------------------
     ...
     * @param   array   分页设置eg:array('index'=>1,'size'=>10)
    ...
    public function get_page($table,$fields,$where=null,$order=array(),$page=array('index'=>1,'size'=>10),$group=array(),$join=array()){
            ...
            if(is_array($where)==true){
                ...
                foreach ($where as $item){
                    ...
                    // if(strlen($field_value)>0){
                        switch($option_type){
                            case 'like':
                                array_push($arr, $field_name." like '%".$this->escape_like_str($field_value)."%'");
                                break;
                            case 'not like':
                                array_push($arr, $field_name." not like '%".$this->escape_like_str($field_value)."%'");
                                break;
                            case 'like_r':
                                array_push($arr, $field_name." like '%".$this->escape_like_str($field_value)."'");
                                break;
                            ...
                            case 'in':
                            case 'not in':
                                array_push($arr, $field_name.' '.$option_type.'('.$this->escape_like_str($field_value).")");
                                break;
                            default:
                                array_push($arr, $field_name.' '.$option_type.' '.$this->escape($field_value));
                        }
                    // }
                }
                $str=implode(' and ',$arr);
        ...
        $query=$this->query('SELECT FOUND_ROWS() as rows_count');
     ...
     * @return  array   array('total'=>0,'rows'=>array());
     */
    public function get_list($table, $fields, $where=null, $order=array(), $group=array()){
        $sql='select SQL_CALC_FOUND_ROWS '.$fields.' from '.$table;
